        ...
        public void HtmlDiff(List<string> body, CSVDiff fileDiff)
        {
            ...
            foreach (var diff in fileDiff.Diffs)
            {
                ...
                for (var i = 0; i < outFields.Length; i++)
                {
                    ...
                    var old = "";
                    var newDiff = "";
                    var style = chg == "Add" || chg == "Delete" ? chg.ToLower() : "";
...
#elif NET47 || NET48
                        body.Add($"<span class='delete'><code>{System.Net.WebUtility.HtmlEncode(old)}</code></span>");
#endif
                    }
...
#elif NET47 || NET48
                    body.Add($"<span{(string.IsNullOrEmpty(style) ? "" : $" class='{style}'")}><code>{System.Net.WebUtility.HtmlEncode(newDiff)}</code></span>");
#endif
                    body.Add("</td>");
                }
                body.Add("</tr>");
